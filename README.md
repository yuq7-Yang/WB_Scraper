...
| 智能话术 | 本地免费 LLM（Ollama qwen2.5）先判断真实意向，再按评论需求生成针对性话术；模型不可用时退回关键词/评论模板 |
...
# 免费本地 LLM：先判断评论是否有真实意向，再按评论需求生成自动回复
...
开启 `LOCAL_LLM_ENABLED=true` 后，自动匹配话术会先把采集关键词和拉取到的评论发给本地模型，让模型判断是否有真实意向；只有判断有意向时，才继续让模型理解评论需求并生成回复。模型没启动或不可用时，面板会退回 `config.py` 里的关键词/评论模板，不会影响基础流程。
...
开启本地 LLM 后，发送前会先让免费本地模型判断评论是否有真实意向；判断为无意向会跳过（记为 `skipped`）。判断为有意向后，再根据拉取到的评论需求生成回复。未开启或模型不可用时，系统使用关键词 + 评论内容的模板匹配兜底。
